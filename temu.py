...
def auto_play_game(link):
    try:
        ...
        return f"✅ Played {game_type} | {title}" if success else f"⚠️ Opened page but no action."
...
def setup_bot(bot):
    ...
    @bot.command()
    async def shein(ctx): await claim(ctx, "https://onelink.shein.com/15/your-shein-link")
