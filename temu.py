...
async def boost_temu_link(link, discord_channel=None):
    for attempt in range(1, 3):  # Retry up to 3 times
        ...
        try:
            ...
            # Click claim/join button if present
            try:
                join_button = await page.waitForSelector('button, a', timeout=5000)
                if join_button:
                    await join_button.click()
                    print("✅ Clicked claim/join button")
            except:
                print("⚠️ No clickable button found")
